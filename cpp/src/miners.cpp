        ...
        for (std::size_t j = i + 1; j < prev_frequents.size(); ++j) {
            ...
            for (std::size_t remove_index = 0; remove_index < candidate.size(); ++remove_index) {
                ...
                for (std::size_t idx = 0; idx < candidate.size(); ++idx) {
                    if (idx != remove_index) {
                        if (!subset_encoded.empty()) subset_encoded.push_back(',');
...
std::vector<int> count_supports_parallel(const Dataset& dataset, const std::vector<Itemset>& candidates, unsigned workers) {
